...
if !argv._[0] or ( _(actions).contains(argv._[0]) and !argv.function )
  throw new Error('`function` argument is require with your function name.')

env = 'staging'
env = 'production' if argv.production

configFile = "./config/#{argv.function}/config-#{env}.yml"
...
gulp.task 'coffee', ->
  gulp.src('./src/**/*.coffee')
    .pipe wait(1500)
    .pipe coffee(bare: true).on('error', (err)-> console.log(err, err.stack) )
    .pipe gulp.dest('dist/')
